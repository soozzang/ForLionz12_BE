...
public class AssignmentController {
    ...
    public ApiResponse<AssignmentIdResponse> createAssignment(@RequestBody AssignmentRequest request) {
        return ApiResponse.createSuccess(ASSIGNMENT_CREATED_SUCCESS,assignmentService.addAssignment(SecurityUtil.getCurrentMemberId(), request));
    }

    @Operation(summary = "과제물 수정")
    @PutMapping("{id}")
    public ApiResponse<AssignmentIdResponse> updateAssignment(@PathVariable("id") Long id, @RequestBody AssignmentRequest request) {
        return ApiResponse.createSuccess(ASSIGNMENT_UPDATED_SUCCESS,assignmentService.updateAssignment(id,request));
    }

    @Operation(summary = "파트별 과제공지 조회" , description = "part에는 BE/FE/ALL이 들어갈 수 있습니다.")
    @GetMapping("/part/{part}")
    public ApiResponse<List<AssignmentResponse>> findAssignmentByPart(@PathVariable("part") String part) {
        return ApiResponse.createSuccess(GET_ASSIGNMENT_SUCCESS,assignmentService.findAssignmentByPart(part));
    }

    @Operation(summary = "모든 과제 공지 조회")
    @GetMapping("all")
    public ApiResponse<List<AssignmentResponse>> findAllAssignment() {
        return ApiResponse.createSuccess(GET_ASSIGNMENT_SUCCESS, assignmentService.findAllAssignments());
    }

    @Operation(summary = "id로 과제공지 상세 조회")
    @GetMapping("{id}")
    public ApiResponse<AssignmentResponse> getAssignmentDetail(@PathVariable("id") Long id) {
        return ApiResponse.createSuccess(GET_ASSIGNMENT_SUCCESS, assignmentService.findAssignmentById(id));
    }

    @Operation(summary = "특정 과제의 id로 모든 과제 제출물 조회")
    @GetMapping("{id}/submissions")
    public ApiResponse<List<SubmissionResponse>> getSubmissions(@PathVariable("id") Long id) {
        return ApiResponse.createSuccess(GET_ASSIGNMENT_SUCCESS, assignmentService.findSubmissionsByAssignment(id));
    }
